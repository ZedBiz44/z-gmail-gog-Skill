...
Date: 2026-09-04 | Prepared by: Manus | Status: Draft, ready for Cody’s technical review
...
| Authoritative branch | `main` unless Jack approves another branch |
...
| First test | Terry and Harry, after Cody confirms both technical connections and their profile locations. |
...
| Structural validator | `python3 /home/ubuntu/z-ai-skill-developer-Skill/scripts/validate_skill.py --repository /path/to/z-gmail-gog-Skill` |
...
| Item | Decision |
|---|---|
| Default email mode | `draft_only` until Jack and Cody approve send-enabled testing for the individual agent. |
| Human approver | The role named in the active profile. Jack approves the first-test results and wider rollout. |
| Technical operator | Cody sets up GOG, OAuth, access controls, profile locations, and the skill installation. |
| Wider rollout | Do not install for additional agents until Terry and Harry both complete the agreed tests, the results are recorded, and Jack approves the next step. |
...
| Approved technical boundary | Use only the GOG actions and OpenClaw permissions provided by Cody. Do not expose credentials. |
| Last known-good commit | Record immediately before the first test. |
| Rollback owner | Jack makes the operating decision. Cody performs the technical change. |
...
| Terry profile loads correctly | Pending technical connection |
| Harry profile loads correctly | Pending technical connection |
| First-test records completed | Pending |
| GOG draft-only block proven | Pending |
| Send-enabled routine email test approved and proven | Pending Jack and Cody approval |
| Notion operating SOP created | Pending |
| GitHub issue and journal entry updated | Pending |
| Jack approves next stage | Pending |
